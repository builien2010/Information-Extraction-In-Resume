...
def extract_chars(filepath, filename, output_dir):
	output_filepath = output_dir + char_output_dir+filename+"_chars.csv"
	pdfplumber_cmd = "pdfplumber --types char < '"+filepath+"' > '"+output_filepath+"'"
	subp.check_call(pdfplumber_cmd, shell=True)
	return output_filepath

def group_chars_into_words(filepath, filename, output_dir):
	output_filepath = output_dir + words_output_dir+filename+"_words.csv"
...
def populate_boxes_on_pdf1(filepath, filename, result, img_origin_dir):
	images = convert_from_path(filepath)
	...
	for page in result:
		...
		cv2.imwrite(img_origin_dir+"/"+filename+"-"+str(page_no)+".jpg", image)
